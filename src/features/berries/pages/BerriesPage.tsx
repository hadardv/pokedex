...
import FirmnessSlider, {
  type FirmnessOption,
  type FirmnessValue,
} from "../components/FirmnessSlider";
...
export default function BerriesPage() {
  ...
  const countsByFirmness = useMemo(() => {
    ...
    (berries ?? []).forEach((b) => map.set(b.firmness, (map.get(b.firmness) ?? 0) + 1));
    return map;
  }, [berries]);
        ...
        <main className={styles.collection}>
          ...
          <div className={styles.scrollList}>
            {filtered.length === 0 ? (
              ...
              filtered.map((b) => <BerryCard key={b.id} berry={b} />)
            )}
          </div>
        </main>
      </div>
